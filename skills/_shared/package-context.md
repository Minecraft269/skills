...
github-pr-manager 克隆 PR → 触发联动 → universal-project-kickoff (深度 1)
  → 初始化完成 → 触发联动 → proactive-skill-discovery (深度 2)
    → 发现缺失插件 → 触发联动 → 深度已达上限，跳过
```
...
如果健康检查发现 `proactive-skill-discovery` 的 SKILL.md 格式异常（该技能被 4/5 的技能依赖），其他技能应：
...
2. universal-project-kickoff 完成 CLAUDE.md 生成
   → 二级联动: 推荐 proactive-skill-discovery 扫描技术栈
   
3. proactive-skill-discovery 发现缺失 MCP 工具
   → 三级联动: 应推荐 quick-plugin-installer → ❌ 截断
